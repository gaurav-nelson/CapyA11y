...
export function formatEvidenceReport(
  ...
  if (apply) {
    lines.push(`| Auto-applied (this run) | ${apply.applied.length}${apply.dryRun ? " (dry-run)" : ""} |`);
  ...
  if (apply && apply.applied.length > 0) {
    ...
    lines.push("No fixes applied in this report. Run `capya11y fix --safe` to remediate structural issues.");
    lines.push("");
  }
  ...
  if (suggest.length > 0) {
    ...
    lines.push("");
    lines.push("These require accessible name / copy judgment. Prefer Cursor `capya11y-fix` skill for proposals.");
    lines.push("");
